	  ...
	  },
	  {
		...
		public async Task<IAssessmentResponse<AssessmentGroupMember>> AddMember(int profileId, int groupId)
		{
			try
			{
				...
				{
					await AddNotificationAsync(member.ProfileId, ActionType.Assessment, member.Id, ParentType.AssessmentGroup, userId, notifyText);
				}
		...
		public async Task<IAssessmentResponse<AssessmentIndividualToolReportView>> GenerateIndividualProfileReport(int ProfileAssessmentID)
		{
			try
			{
				...
				if (profileAssessment != null)
				{
					...
					{
						English = profileAssessment.Profile.FirstNameEn + " " + profileAssessment.Profile.SecondNameEn + " " + profileAssessment.Profile.LastNameEn,
						Arabic = profileAssessment.Profile.FirstNameAr + " " + profileAssessment.Profile.SecondNameAr + " " + profileAssessment.Profile.LastNameAr
					};
		...
		public async Task<IAssessmentResponse<bool>> MakeCoordinator(int profileID)
		{
			try
			{
				...
				if (customNotificationData?.isEnabled == true || customNotificationData == null)
				{
					await AddNotificationAsync(member.ProfileId, ActionType.AssessmentCoordinator, member.Id, ParentType.AssessmentGroup, userId, notifyText);
				}
		...
		private bool IsCompleted(int id, int profileID)
		{
			return _appDbContext.ProfileAssessmentToolScores.Any(p => p.AssessmentToolId == id && p.ProfileId == profileID && p.IsCompleted == true);
		}

		private bool IsFailed(int id, int profileID)
		{
			return _appDbContext.ProfileAssessmentToolScores.Any(p => p.AssessmentToolId == id && p.ProfileId == profileID && p.IsFailed == true);
		}
