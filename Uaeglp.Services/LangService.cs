...
namespace Uaeglp.Services
{
    ...
    public class LangService : ILangService
    {
        ...
        public async Task<Languages_Label> UpdatelabelAsync(ViewModels.PageData model)
        {
            var data = await _appContext.Languages_Label.Where(x => x.Id == model.Id).FirstOrDefaultAsync();

            if(data != null)
            {
                //data.Labelname = model.LabelName;
                //data.Pagename = model.PageName;
                //data.Page_Displayname = model.PageDisplayName;
        ...
        {
            var data = await _appContext.Languages_Label.Where(x => x.Id == labelid).FirstOrDefaultAsync();

            var isDeleted = false;

            if (data != null)
            {
                //_appContext.Languages_Label.Remove(data);
                data.Is_Active = 0;
                await _appContext.SaveChangesAsync();
                isDeleted = true;
            }
