...
namespace Uaeglp.ViewModels.ProgramViewModels
{
    public class ProgramView
    {
        ...
        public int? BatchId => ActiveBatch?.Id ?? BatchView?.Id;

        public BatchView BatchView => Batches.OrderByDescending(k =>k.DateRegTo).FirstOrDefault();

        public List<PublicProfileView> CompletedUsersList { get; set; }

        public bool HasActiveBatches => Batches.Any(k => !k.IsClosed && k.DateRegFrom <= DateTime.Now.Date && k.DateRegTo >= DateTime.Now.Date);
        ...
        public LookupItemView ApplicationStatus
        {
            get
            {
                ...
                var pro = Applications.FirstOrDefault(k => k.BatchId == batchId);
        ...
        public decimal ApplicationCompletedPercentage
        {
            get
            {
                ...
                var pro = Applications.FirstOrDefault(k => k.BatchId == ActiveBatch.Id);
